...
public class VikingSportController : Controller
{
   ...
   public async Task<IActionResult> CreateNewWorkout(Workout workout)
   {
      try
      {
         var newWorkout = _rWorkout.CreateWorkout(workout,
            Guid.Parse(this.User.Claims.First(t => t.Type == "userId").Value.ToString()));
   ...
   public async Task<IActionResult> CreateNewExercise(Exercise exercise)
   {
      try
      {
   ...
   public async Task<IActionResult> DelExercises(List<Exercise> exercises)
   {
      try
      {
